...
uint8_t MCP2515_Init(const uint8_t canIDMode, const uint8_t canSpeed, const uint8_t canClock)
{
  ...
  // Set up clock output signal
  MCP2515_ModifyRegister(MCP_CANCTRL, CLKOUT_ENABLE, CLKOUT_ENABLE);  // Enable clock
  MCP2515_ModifyRegister(MCP_CANCTRL, CLKOUT_PS8, CLKOUT_PS2);        // Set prescaler to 2
  MCP2515_ModifyRegister(MCP_CNF3, SOF_ENABLE, SOF_DISABLE);          // Disable alternate pin function
